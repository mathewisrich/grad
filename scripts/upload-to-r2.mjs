#!/usr/bin/env node
/**
 * Compress every photo in 113D3500/ and upload to Cloudflare R2.
 *
 * For each source photo we produce two copies:
 *   1. full/<name>.jpg   - 6000x4000 at quality 85 (downloadable, ~1-1.5 MB)
 *   2. thumb/<name>.jpg  - 800px wide at quality 80 (gallery grid, ~60 KB)
 *
 * A manifest.json with the list of photos is generated locally and copied
 * to the Next.js app's /lib folder so the gallery knows what to render.
 *
 * Run with:  npm run upload
 */
...
const SRC_DIR = path.join(rootDir, "113D3500");
const MANIFEST_DST = path.join(rootDir, "lib", "manifest.json");
...
async function processOne(filename, idx, total) {
  ...
  if (fullExists && thumbExists) {
    ...
    return { name: base, full: fullKey, thumb: thumbKey };
  }
...
async function main() {
  ...
  console.log(
    `\nDone. ${photos.length} photos in R2. Manifest written to ${MANIFEST_DST}`
  );
}
